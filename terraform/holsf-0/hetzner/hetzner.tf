        ...
        vms = {
          "${i + 1}" = {
            labels = "group_name:${vm_group.name},val_start:${vm_group.validator_start + (i * (vm_group.validator_end -
              ...
              vm_group.validator_start) / vm_group.count), vm_group.validator_end)},supernode:${try(vm_group.supernode, can(regex("(super|bootnode|mev)", vm_group.name))) ? "True" : "False"}"
...
resource "local_file" "ansible_inventory" {
  ...
  filename = "../../../ansible/inventories/holsf-0/hetzner_inventory.ini"
}
