...
variable "digitalocean_regions_no_sgp1" {
  default = [
    "nyc1",
    "lon1",
    "nyc3",
  ...
  digitalocean_vm_groups = flatten([
    ...
    vm_group.count > 0 ? [
      for i in range(0, vm_group.count) : {
        ...
        vms = {
          "${i + 1}" = {
            tags = "group_name:${vm_group.name},val_start:${vm_group.validator_start + (i * (vm_group.validator_end - vm_group.validator_start) / vm_group.count)},val_end:${min(vm_group.validator_start + ((i + 1) * (vm_group.validator_end - vm_group.validator_start) / vm_group.count), vm_group.validator_end)},supernode:${can(regex("(super|bootnode|mev)", vm_group.name)) ? "True" : "False"}${vm_group.name == "bootnode" ? ",bootnode:${var.ethereum_network}" : ""}"
            region = try(vm_group.region,
              try(vm_group.size, can(regex("(super|bootnode)", vm_group.name)) ? "s-8vcpu-32gb-640gb-intel" : "s-8vcpu-16gb") == "s-8vcpu-32gb-640gb-intel" ?
            ...
            size = try(vm_group.size, can(regex("(super|bootnode)", vm_group.name)) ? "s-8vcpu-32gb-640gb-intel" : "s-8vcpu-16gb")
...
resource "local_file" "ansible_inventory" {
  ...
  filename = "../../ansible/inventories/sepsf-0/inventory.ini"
}